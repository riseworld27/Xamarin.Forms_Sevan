...
namespace ComponentProSamples
{
	...
	public class SiteManagerLogic
	{
		...
		{
			return CheckConnectionParameters (_view, connection);
		}
		...
		/// <param name="connection">The connection information.</param>
		public static bool CheckConnectionParameters(IView view, IConnectionInfo connection)
		{
			// Check parameters.
			if (string.IsNullOrEmpty(connection.Server))
			{
				view.ShowMessage("Please specify server to connect to.", "Server address cannot be empty.", MessageBoxType.Error, false);
				return false;
			}

			if (connection.Port <= 0)
			{
				view.ShowMessage("Please specify a valid server port.", "Invalid port.", MessageBoxType.Error, false);
				return false;
			}
